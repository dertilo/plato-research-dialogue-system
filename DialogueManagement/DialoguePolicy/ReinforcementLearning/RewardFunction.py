...
class SlotFillingReward(Reward):
    ...
    def calculate(
        self, state, actions, goal=None, force_terminal=False, agent_role="system"
    ):
        """
        Calculate the reward to be assigned for taking action from state.
        ...
        # task_success = self.calc_task_success_after_Liu_and_Lane(agent_role, goal,
        #                                                          state)
        task_success = None  # Tilo: we are not using this!
        return reward, dialogue_success, task_success

    def calc_task_success_after_Liu_and_Lane(self, agent_role, goal, state):
        task_success = None
        if agent_role == "system":
            ...
            for slot in goal.constraints:
                ...
                if goal.ground_truth:
                    if (
                        goal.ground_truth[slot] != goal.constraints[slot].value
                        and goal.constraints[slot].value != "dontcare"
                    ):
                        task_success = False
                        break
                ...
                # Fall back to the noisier signal, that is the slots filled.
                    ...
                    slot in state.slots_filled
                    and state.slots_filled[slot] != goal.constraints[slot].value
                    and goal.constraints[slot].value != "dontcare"
                ):
                    task_success = False
                    break
    ...
    def _evaluate_offer(self, agent_role, goal, reward, state):
        ...
        for constr in goal.constraints:
            if goal.ground_truth:
                # Multi-agent case
                if (
                    goal.ground_truth[constr] != goal.constraints[constr].value
...
class SlotFillingGoalAdvancementReward(Reward):
    ...
    def calculate(
        ...
        if goal is None:
            print(
                "Warning: SlotFillingGoalAdvancementReward() called without " "a goal."
            )
        ...
        # Check if the goal has been advanced
            ...
            if sum(
                [
                    1 if self.prev_state.slots_filled[s] else 0
                    for s in self.prev_state.slots_filled
                ]
            ...
            # Or if the new state has more requests filled than the old one
                ...
                [
                    1 if self.prev_goal.actual_requests[r] else 0
                    for r in self.prev_goal.actual_requests
                ]
            ) < sum(
                [1 if goal.actual_requests[r] else 0 for r in goal.actual_requests]
            ):
                reward = 1
        ...
        if agent_role == "system":
            ...
            for slot in goal.constraints:
                ...
                # Fall back to the noisier signal, that is the slots filled.
                    ...
                    state.slots_filled[slot] != goal.constraints[slot].value
                    and goal.constraints[slot].value != "dontcare"
                ):
                    task_success = False
